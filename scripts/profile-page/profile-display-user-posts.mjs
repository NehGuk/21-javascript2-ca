export function displayUserPosts(userPosts) {
  ...
  for (let i = 0; i < userPosts.length; i++) {
    ...
    const formattedDate = new Date(userPosts[i].created).toLocaleDateString(
      "en-us",
      { month: "short", day: "numeric" }
    );
    const formattedTime = new Date(userPosts[i].created).toLocaleTimeString(
      "en-GB"
    );
    ...
    myPostsContainer.innerHTML += `
        ...
        <div class="card-footer">
            <a href="#"><img src="${avatar}" width="32" height="32" class="rounded-circle border border-primary" style="width: 32; height: 32; object-fit: cover; border-width: 1px !important;" alt="${userPosts[i].owner} avatar"></a>
